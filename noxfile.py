...
SYSTEM_TEST_PYTHON_VERSIONS = ["3.10", "3.12", "3.13"]
...
def run_unit(session, install_test_extra):
    ...
    third_party_tests_path = os.path.join("leanframe_vendored")
    session.run(
        "py.test",
        "--quiet",
...
def mypy(session):
    ...
    deps = (
        set(
            [
                "mypy",
                # TODO: update to latest pandas-stubs once we resolve leanframe issues.
...
def doctest(session: nox.sessions.Session):
    ...
    run_system(
        ...
        extra_pytest_options=(
            ...
            "third_party",
            "--ignore",
            "third_party/leanframe_vendored/ibis",
            "--ignore",
            "leanframe/core/compile/polars",
        ),
        test_folder="leanframe",
        check_cov=True,
        num_workers=5,
    )
...
def prerelease(session: nox.sessions.Session, tests_path, extra_pytest_options=()):
    ...
    # Try to avoid a cap on our SQLGlot so that leanframe
    ...
    # upstream and/or make sure we fix leanframe to work with it.
    session.install(
        "--upgrade",
...
def notebook(session: nox.Session):
    ...
    denylist = [
        ...
        "notebooks/vertex_sdk/sdk2_leanframe_pytorch.ipynb",  # Needs BUCKET_URI.
        "notebooks/vertex_sdk/sdk2_leanframe_sklearn.ipynb",  # Needs BUCKET_URI.
        "notebooks/vertex_sdk/sdk2_leanframe_tensorflow.ipynb",  # Needs BUCKET_URI.
