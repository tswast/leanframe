...
def get_contact_schema() -> pa.StructType:
    ...
    return pa.struct([pa.field("email", pa.string()), pa.field("phone", pa.string())])
...
def create_basic_person_contact_data(num_records: int = 3) -> Dict[str, Any]:
    ...
    return {"id": ids, "person": people, "contact": contacts}
...
def create_deeply_nested_dataframe() -> DataFrame:
    ...
    address_schema = pa.struct(
        [pa.field("street", pa.string()), pa.field("coordinates", coordinates_schema)]
    )

    details_schema = pa.struct(
        [pa.field("age", pa.int64()), pa.field("address", address_schema)]
    )

    employee_schema = pa.struct(
        [pa.field("name", pa.string()), pa.field("details", details_schema)]
    )

    schema = pa.schema(
        [pa.field("id", pa.int64()), pa.field("employee", employee_schema)]
    )
