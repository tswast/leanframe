...
def main():
    ...
    print(
        "   - Orders: email at shipping.recipient.email (nested 2 levels, different path!)"
    )
    ...
    nested.add(name="orders", df=orders_lf, table_qualifier="local_duckdb.main.orders")
    ...
    print("\n📊 Results comparison:")
    print(
        f"   Simple join: {len(simple_pandas)} rows, {len(simple_pandas.columns)} columns"
    )
    print(
        f"   Power user: {len(summary_pandas)} rows, {len(summary_pandas.columns)} columns (aggregated)"
    )
    ...
    temp_df = session.DataFrame(pd.DataFrame({"id": [1, 2, 3], "value": [10, 20, 30]}))
    ...
    print(
        "   - Joined on nested email: profile.contact.email ⋈ shipping.recipient.email"
    )
    ...
    print(
        "   1. DataFrameHandler owns its backend reference (table_qualifier property)"
    )
    ...
    print(
        "   4. Handlers can independently update backend status via set_table_qualifier()"
    )
    print(
        "   5. Clean lifecycle: in-memory → save (set qualifier) → drop (clear qualifier)"
    )
