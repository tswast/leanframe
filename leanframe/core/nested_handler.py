...
class DynamicNestedHandler:
        ...
        for column_name in ibis_table.columns:
            try:
                ...
                if column_type.is_struct():
                    ...
                    self._analyze_struct_column_native(ibis_table[column_name], column_name, column_name, depth=0)
                else:
                    print(f"      → Regular column, keeping as-is")
                    
            except Exception as e:
                print(f"   ⚠️  Could not analyze column '{column_name}': {e}")
    
    def _analyze_struct_column_native(self, struct_expr, field_path: str, root_column: str, depth: int):
        ...
        if hasattr(struct_type, 'fields'):
            ...
            for field_name, field_type in struct_type.fields.items():
                ...
                extracted_name = f"{root_column}_{field_name}" if depth == 0 else f"{field_path.replace('.', '_')}_{field_name}"
                ...
                try:
                    ...
                    if field_type.is_struct():
                        ...
                        self._analyze_struct_column_native(field_expr, current_path, root_column, depth + 1)
                    else:
                        # This is a leaf field we can extract
                        print(f"{indent}    ✅ Extractable: {current_path} → {extracted_name}")
    ...
    def extracted_fields(self) -> Dict[str, str]:
        ...
        return {path: info['extracted_name'] for path, info in self.nested_fields.items()}
