...
def get_cloud_function_name(def_, uniq_suffix=None):
    ...
    cf_name = f"leanframe-{cf_name}"  # for identification
    if uniq_suffix:
...
def get_remote_function_name(def_, uniq_suffix=None):
    ...
    bq_rf_name = f"leanframe_{bq_rf_name}"  # for identification
    if uniq_suffix:
...
def remote_function(
    ...
    Args:
        ...
        session : leanframe.Session, Optional
    ...
    **Prerequisites:**
    ...
    if not bigquery_connection:
        if session:
            bigquery_connection = session._bigquery_connection
    if not bigquery_connection:
        raise ValueError(
