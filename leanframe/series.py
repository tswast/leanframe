...
from leanframe.core import WindowSpec
import leanframe.core.blocks as blocks
import leanframe.core.indexes.implicitjoiner
import leanframe.core.indexes.index
from leanframe.core.ordering import OrderingColumnReference, OrderingDirection
import leanframe.core.window
import leanframe.dtypes
from leanframe.guid import generate_guid
import leanframe.indexers
import leanframe.operations as ops
import leanframe.operations.base
import leanframe.operations.strings as strings
import leanframe.scalar


class Series(leanframe.operations.base.SeriesMethods):
    ...
    @property
    def index(self) -> leanframe.core.indexes.implicitjoiner.ImplicitJoiner:
        """The index of the series."""
        return self._viewed_block.index

    @property
    def loc(self) -> leanframe.indexers.LocSeriesIndexer:
        """Set items by index label.

        No get or slice support currently supported.
        """
        return leanframe.indexers.LocSeriesIndexer(self)

    @property
    def iloc(self) -> leanframe.indexers.IlocSeriesIndexer:
        """Get items by slice.

        No set operations currently supported.
        """
        return leanframe.indexers.IlocSeriesIndexer(self)
    ...
    def astype(
        self,
        dtype: Union[
            leanframe.dtypes.BigFramesDtypeString, leanframe.dtypes.BigFramesDtype
        ],
    ) -> Series:
        """Casts the series to another compatible dtype."""
        ibis_dtype = leanframe.dtypes.leanframe_dtype_to_ibis_dtype(dtype)
        return self._apply_unary_op(leanframe.operations.AsTypeOp(ibis_dtype))
    ...
    def drop(self, labels: str | typing.Sequence[str]):
        ...
        if isinstance(block._index, leanframe.core.indexes.Index):
    ...
    def cumsum(self) -> Series:
        ...
        return self._apply_window_op(
            agg_ops.sum_op, leanframe.core.WindowSpec(following=0)
        )

    def cummax(self) -> Series:
        """The cumulative maximum of values in the series."""
        return self._apply_window_op(
            agg_ops.max_op, leanframe.core.WindowSpec(following=0)
        )

    def cummin(self) -> Series:
        """The cumulative minimum of values in the series."""
        return self._apply_window_op(
            agg_ops.min_op, leanframe.core.WindowSpec(following=0)
        )
    ...
    def rank(
        ...
        if method == "first":
            ...
            rank_id = f"{self._value_column}_leanframe_rank"
            agg_op = {
    ...
    def mean(self) -> float:
        ...
        Returns:
            ...
            :func:`~leanframe.scalar.Scalar.compute()`.
        """
    ...
    def _align(self, other: typing.Any, how="outer") -> tuple[ibis_types.Value, ibis_types.Value, leanframe.core.indexes.implicitjoiner.ImplicitJoiner]:  # type: ignore
    ...
    def _align3(self, other1: typing.Any, other2: typing.Any, how="left") -> tuple[ibis_types.Value, ibis_types.Value, ibis_types.Value, leanframe.core.indexes.implicitjoiner.ImplicitJoiner]:  # type: ignore
    ...
    def _align_n(
        ...
        leanframe.core.indexes.implicitjoiner.ImplicitJoiner,
    ]:
        values = [self._value]
        index = self.index
        for other in others:
            ...
            elif isinstance(other, leanframe.scalar.DeferredScalar):
    ...
    ) -> leanframe.scalar.ImmediateScalar:
        aggregation_result = typing.cast(
            ibis_types.Scalar, op._as_ibis(self[self.notnull()]._to_ibis_expr())
        )
        return leanframe.scalar.DeferredScalar(aggregation_result).compute()
    ...
    def rolling(self, window: int, *, min_periods=None) -> leanframe.core.window.Window:
        ...
        return leanframe.core.window.Window(
            self._block.copy(), window_spec, self._value_column, self.name
        )

    def expanding(self, min_periods: int = 1) -> leanframe.core.window.Window:
    ...
    def mask(self, cond, other=None) -> Series:
        ...
        if not isinstance(cond, Series):
            raise TypeError(
                f"Only leanframe series condition is supported, received {type(cond).__name__}"
            )
        return self.where(~cond, other)

    def to_frame(self) -> leanframe.DataFrame:
        ...
        return leanframe.DataFrame(block.index, col_names)
    ...
    def str(self) -> strings.StringMethods:
        ...
        Returns:
            leanframe.operations.strings.StringMethods: Methods that act on a string Series.
        """
...
class SeriesGroupyBy:
    ...
    def cumsum(self) -> Series:
        ...
        return self._apply_window_op(
            agg_ops.sum_op,
            leanframe.core.WindowSpec(grouping_keys=(self._by,), following=0),
        )
...
def _interpret_as_ibis_literal(value: typing.Any) -> typing.Optional[ibis_types.Value]:
    if (
        ...
        or isinstance(value, leanframe.scalar.DeferredScalar)
