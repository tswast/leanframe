...
def arrow_dtype_to_bigframes_dtype(arrow_dtype: pa.DataType) -> Dtype:
    ...
    raise TypeError(
        f"Unexpected Arrow data type {arrow_dtype}."
    )
...
def bigframes_dtype_to_arrow_dtype(
    ...
    if isinstance(bigframes_dtype, pd.ArrowDtype):
        ...
        raise TypeError(
            f"No arrow conversion for {bigframes_dtype}."
        )
...
def arrow_type_to_literal(
    ...
    raise TypeError(
        f"No literal  conversion for {arrow_type}."
    )
...
def bigframes_type(dtype) -> Dtype:
    ...
    if _is_bigframes_dtype(dtype):
        ...
        raise TypeError(
            f"Cannot infer supported datatype for: {dtype}."
        )
...
def _infer_dtype_from_python_type(type: type) -> Dtype:
    ...
    if issubclass(type, datetime.time):
        ...
        raise TypeError(
            f"No matching datatype for python type: {type}."
        )
...
def lcd_type_or_throw(dtype1: Dtype, dtype2: Dtype) -> Dtype:
    ...
    if result is None:
        raise NotImplementedError(
            f"BigFrames cannot upcast {dtype1} and {dtype2} to common type."
        )
    return result
