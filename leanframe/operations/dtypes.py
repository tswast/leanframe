...
def ibis_dtype_to_leanframe_dtype(
...
def leanframe_dtype_to_ibis_dtype(
    leanframe_dtype: Union[BigFramesDtypeString, BigFramesDtype]
) -> IbisDtype:
    """Converts a BigFrames supported dtype to an Ibis dtype

    Args:
        leanframe_dtype: A dtype supported by BigFrames
    ...
    type_string = str(leanframe_dtype)
    if type_string in BIGFRAMES_STRING_TO_BIGFRAMES:
        leanframe_dtype = BIGFRAMES_STRING_TO_BIGFRAMES[
            typing.cast(BigFramesDtypeString, type_string)
        ]
    else:
        raise ValueError(f"Unexpected data type {leanframe_dtype}")

    return BIGFRAMES_TO_IBIS[leanframe_dtype]
