...
class Pipeline(leanframe.ml.api_primitives.BaseEstimator):
    ...
    Currently in leanframe.ml only two step pipelines are supported. The first
    step should be preprocessing, and the second step should be the model."""

    def __init__(
        self, steps: List[Tuple[str, leanframe.ml.api_primitives.BaseEstimator]]
    ):
        self.steps = steps
        ...
        if isinstance(
            transform,
            (
                leanframe.ml.compose.ColumnTransformer,
        ...
        if isinstance(
            estimator,
            (leanframe.ml.linear_model.LinearRegression, leanframe.ml.cluster.KMeans),
        ):
    ...
    def fit(self, X: leanframe.DataFrame, y: Optional[leanframe.DataFrame] = None):
        """Fit each estimator in the pipeline to the transformed output of the
        previous one. In leanframe.ml this will compile the pipeline to a single
        ...
        if isinstance(self._estimator, leanframe.ml.cluster.KMeans):
    ...
    def score(
        self,
        X: Optional[leanframe.DataFrame] = None,
        y: Optional[leanframe.DataFrame] = None,
    ):
        ...
        if isinstance(self._estimator, leanframe.ml.linear_model.LinearRegression):
