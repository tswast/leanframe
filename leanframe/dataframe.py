...
import leanframe.core.type_checks
import leanframe.series
from leanframe_vendored.pandas.core import frame as vendored_pandas_frame
from leanframe_vendored.pandas.pandas import _typing as vendored_pandas_typing
...
class DataFrame(vendored_pandas_frame.DataFrame):
    ...
    # Must be above 5000 for pandas to delegate to leanframe for binops
    ...
    def __init__(
        ...
        *,
        ...
        dtype: typing.Optional[
            leanframe.dtypes.DtypeString | leanframe.dtypes.Dtype
        ] = None,
        backend: typing.Optional[ibis.backends.BaseBackend] = None,
    ):
        # Ignore object dtype if provided, as it provides no additional
        # information about what BigQuery type to use.
        if dtype is not None and leanframe.dtypes.is_object_like(dtype):
            dtype = None
        ...
        if isinstance(data, ibis.expr.types.Table):
            ...
            table_expr = data._get_table()

        # Dict of Series
        elif (
            leanframe.core.type_checks.is_dict_like(data)
            and len(data) >= 1
            and any(
                isinstance(data[key], leanframe.series.Series) for key in data.keys()
            )
        ):
            if not all(
                isinstance(data[key], leanframe.series.Series) for key in data.keys()
                ...
                raise NotImplementedError(
                    f"Cannot mix Series with other types."
                )
            keys = list(data.keys())
            first_series = data[keys[0]]
            column_expr = (
                typing.cast(leanframe.series.Series, first_series)
                ._get_column()
            )

            for key in keys[1:]:
                other = typing.cast(leanframe.series.Series, data[key])
        ...
        if table_expr:
            ...
            if dtype:
                bf_dtype = leanframe.dtypes.bigframes_type(dtype)
            ...
            table_expr = ibis.memtable(pd_dataframe)
    ...
    def __dir__(self):
        return dir(type(self)) + [
            label
            for label in self._table_expr.columns
    ...
    def dtypes(self) -> pandas.Series:
        return pandas.Series(
            # TODO: convert ibis types to ArrowDtype.
            data=[pandas.ArrowDtype(pyarrow.string()) for _ in self._table_expr.columns],
            index=self._table_expr.columns,
        )
    ...
    def _getitem_label(self, key: blocks.Label):
        ...
        if len(col_ids) == 1:
            return leanframe.series.Series(block)
        return DataFrame(block)
    ...
    def _repr_html_(self) -> str:
        ...
        if bigframes.options.experiments.blob:
            blob_cols = [
                ...
                if df[col].dtype == leanframe.dtypes.OBJ_REF_DTYPE
            ]
    ...
    def _apply_binop(
        self,
        other: float | int | leanframe.series.Series | DataFrame,
        op,
        axis: str | int = "columns",
        how: str = "outer",
        reverse: bool = False,
    ):
        if isinstance(other, leanframe.dtypes.LOCAL_SCALAR_TYPES):
    ...
    def _apply_scalar_binop(
        self,
        other: leanframe.dtypes.LOCAL_SCALAR_TYPE,
        op: ops.BinaryOp,
        reverse: bool = False,
    ) -> DataFrame:
        if reverse:
    ...
    def _apply_series_binop_axis_1(
        ...
        if (
            isinstance(other, leanframe.series.Series)
    ...
    def mod(self, other: int | leanframe.series.Series | DataFrame, axis: str | int = "columns") -> DataFrame:  # type: ignore
    ...
    def rmod(self, other: int | leanframe.series.Series | DataFrame, axis: str | int = "columns") -> DataFrame:  # type: ignore
    ...
    def pow(
        self, other: int | leanframe.series.Series, axis: str | int = "columns"
    ) -> DataFrame:
    ...
    def __and__(self, other: bool | int | leanframe.series.Series) -> DataFrame:
    ...
    def __or__(self, other: bool | int | leanframe.series.Series) -> DataFrame:
    ...
    def __xor__(self, other: bool | int | leanframe.series.Series) -> DataFrame:
    ...
    def align(
        self,
        other: typing.Union[DataFrame, leanframe.series.Series],
        join: str = "outer",
        axis: typing.Union[str, int, None] = None,
    ) -> typing.Tuple[
        typing.Union[DataFrame, leanframe.series.Series],
        typing.Union[DataFrame, leanframe.series.Series],
    ]:
        axis_n = utils.get_axis_number(axis) if axis else None
        if axis_n == 1 and isinstance(other, leanframe.series.Series):
            raise NotImplementedError(
    ...
    def update(self, other, join: str = "left", overwrite=True, filter_func=None):
        ...
        if filter_func is not None:  # Will always take other if possible
            ...
            def update_func(
                left: leanframe.series.Series, right: leanframe.series.Series
            ) -> leanframe.series.Series:
    ...
    def combine(
        ...
        func: typing.Callable[
            [leanframe.series.Series, leanframe.series.Series], leanframe.series.Series
        ],
        fill_value=None,
        overwrite: bool = True,
        *,
        ...
        if all([isinstance(val, leanframe.series.Series) for val in results]):
    ...
    def _fast_stat_matrix(self, op: agg_ops.BinaryAggregateOp) -> DataFrame:
        ...
        frame = frame.astype(leanframe.dtypes.FLOAT_DTYPE)
        block = frame._block
    ...
    def corrwith(
        ...
        *,
        ...
        l_block = l_frame.astype(leanframe.dtypes.FLOAT_DTYPE)._block
        r_block = r_frame.astype(leanframe.dtypes.FLOAT_DTYPE)._block

        if isinstance(other, DataFrame):
            block, labels, expr_pairs = l_block._align_both_axes(r_block, how="inner")
        else:
            assert isinstance(other, leanframe.series.Series)
    ...
    @validations.requires_ordering()
    def equals(self, other: typing.Union[leanframe.series.Series, DataFrame]) -> bool:
    ...
    def _assign_series_join_on_index(
        self, label: str, series: leanframe.series.Series
    ) -> DataFrame:
    ...
    def sort_values(
        ...
        *,
        ...
        if isinstance(by, (leanframe.series.Series, indexes.Index, DataFrame)):
            raise KeyError(
    ...
    def items(self):
        ...
        for col_id, col_label in zip(column_ids, column_labels):
            yield col_label, leanframe.series.Series(self._block.select_column(col_id))
    ...
    def where(self, cond, other=None):
        if isinstance(other, leanframe.series.Series):
        ...
        if isinstance(cond, leanframe.series.Series) and cond.name in self_col:
        ...
        for x, self_id in self_col.items():
            ...
            series = leanframe.series.Series(
    ...
    def quantile(
        ...
        if multi_q:
            ...
            return leanframe.series.Series(
    ...
    def count(self, *, numeric_only: bool = False) -> leanframe.series.Series:
        if not numeric_only:
            frame = self
        else:
            frame = self._drop_non_numeric()
    ...
    @validations.requires_index
    @validations.requires_ordering()
    def idxmin(self) -> leanframe.series.Series:
        return leanframe.series.Series(block_ops.idxmin(self._block))

    @validations.requires_index
    @validations.requires_ordering()
    def idxmax(self) -> leanframe.series.Series:
        return leanframe.series.Series(block_ops.idxmax(self._block))
    ...
    def describe(self, include: None | Literal["all"] = None) -> DataFrame:
        if include is None:
            numeric_df = self._select_exact_dtypes(
                leanframe.dtypes.NUMERIC_BIGFRAMES_TYPES_RESTRICTIVE
                + leanframe.dtypes.TEMPORAL_NUMERIC_BIGFRAMES_TYPES
            )
    ...
    def _describe_non_numeric(self) -> DataFrame:
        return typing.cast(
            ...
            self._select_exact_dtypes(
                [
                    leanframe.dtypes.STRING_DTYPE,
                    leanframe.dtypes.BOOL_DTYPE,
                    leanframe.dtypes.BYTES_DTYPE,
                    leanframe.dtypes.TIME_DTYPE,
                ]
            ).agg(["count", "nunique"]),
        )
    ...
    def skew(self, *, numeric_only: bool = False):
        ...
        return leanframe.series.Series(result_block)
    ...
    def pivot_table(
        ...
        if isinstance(agged, leanframe.series.Series):
    ...
    def _drop_non_numeric(self, permissive=True) -> DataFrame:
        numeric_types = (
            set(leanframe.dtypes.NUMERIC_BIGFRAMES_TYPES_PERMISSIVE)
            if permissive
            else set(leanframe.dtypes.NUMERIC_BIGFRAMES_TYPES_RESTRICTIVE)
        )
        non_numeric_cols = [
            col_id
    ...
    def _drop_non_bool(self) -> DataFrame:
        non_bool_cols = [
            col_id
            for col_id, dtype in zip(self._block.value_columns, self._block.dtypes)
            if dtype not in leanframe.dtypes.BOOL_BIGFRAMES_TYPES
        ]
        return DataFrame(self._block.drop_columns(non_bool_cols))

    def _raise_on_non_numeric(self, op: str):
        if not all(
            dtype in leanframe.dtypes.NUMERIC_BIGFRAMES_TYPES_PERMISSIVE
    ...
    def _raise_on_non_boolean(self, op: str):
        if not all(
            dtype in leanframe.dtypes.BOOL_BIGFRAMES_TYPES
            for dtype in self._block.dtypes
        ):
            raise NotImplementedError(
    ...
    def join(
        self,
        other: Union[DataFrame, leanframe.series.Series],
        *,
    ...
    def groupby(
        ...
        by: typing.Union[
            blocks.Label,
            leanframe.series.Series,
            typing.Sequence[typing.Union[blocks.Label, leanframe.series.Series]],
            None,
        ] = None,
        *,
    ...
    def _groupby_series(
        ...
        if not isinstance(by, leanframe.series.Series) and utils.is_list_like(by):
            by = list(by)
        else:
            by = [typing.cast(typing.Union[blocks.Label, leanframe.series.Series], by)]
    ...
    def apply(self, func, *, axis=0, args: typing.Tuple = (), **kwargs):
        ...
        if utils.get_axis_number(axis) == 1:
            ...
            if is_row_processor:
                ...
                bigquery_function_supported_dtypes = (
                    leanframe.dtypes.INT_DTYPE,
                    leanframe.dtypes.FLOAT_DTYPE,
                    leanframe.dtypes.BOOL_DTYPE,
                    leanframe.dtypes.BYTES_DTYPE,
                    leanframe.dtypes.STRING_DTYPE,
                )
                ...
                rows_as_json_series = leanframe.series.Series(
            ...
            if leanframe.dtypes.is_string_like(
                result_series.dtype
            ) and leanframe.dtypes.is_array_like(func.output_dtype):
                import bigframes.bigquery as bbq

                result_dtype = leanframe.dtypes.arrow_dtype_to_bigframes_dtype(
        ...
        if all(
            [
                isinstance(val, leanframe.series.Series) or utils.is_list_like(val)
    ...
    def duplicated(self, subset=None, keep: str = "first") -> leanframe.series.Series:
        if keep is not False:
    ...
    def dot(self, other: _DataFrameOrSeries) -> _DataFrameOrSeries:
        if not isinstance(other, (DataFrame, leanframe.series.Series)):
            raise NotImplementedError(
