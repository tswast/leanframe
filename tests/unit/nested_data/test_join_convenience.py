...
def sample_data(duckdb_backend):
    ...
    customers_table = duckdb_backend.create_table(
        "test_customers", customers_data, temp=True
    )
    orders_table = duckdb_backend.create_table("test_orders", orders_data, temp=True)
    products_table = duckdb_backend.create_table(
        "test_products", products_data, temp=True
    )
...
def test_join_missing_conditions_raises_error(sample_data):
    ...
    with pytest.raises(ValueError, match="Must provide either"):
        handler.join(tables={"c": "customers", "o": "orders"}, how="inner")
...
def test_join_multi_column_conditions(sample_data):
    ...
    table1_data = pd.DataFrame(
        {"id": [1, 2, 3], "region": ["US", "EU", "US"], "value": [10, 20, 30]}
    )

    table2_data = pd.DataFrame(
        {"id": [1, 2, 1], "region": ["US", "EU", "EU"], "amount": [100, 200, 150]}
    )
    ...
    result = handler.join(
        tables={"a": "t1", "b": "t2"},
        on=[("a", "id", "b", "id"), ("a", "region", "b", "region")],
        how="inner",
    )
...
def test_join_cross_join(sample_data):
    ...
    result = handler.join(tables={"c": "customers", "p": "products"}, how="cross")
