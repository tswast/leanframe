...
def create_simple_struct_dataframe():
    ...
    schema = pa.schema([pa.field("id", pa.int64()), pa.field("person", person_schema)])
    ...
    address_schema = pa.struct(
        [
            ...
            pa.field(
                "coordinates",
                pa.struct(
                    [pa.field("lat", pa.float64()), pa.field("lng", pa.float64())]
                ),
            ),
        ]
    )
    ...
    project_schema = pa.struct(
        [
            ...
            pa.field(
                ...
                pa.list_(
                    pa.struct(
                        [pa.field("name", pa.string()), pa.field("email", pa.string())]
                    )
                ),
            ),
            pa.field(
...
def demonstrate_accessing_nested_data():
    ...
    df_temp = pa.table([person_array], names=["person"]).to_pandas(
...
def working_with_pandas_extension_types():
    ...
    data = {
        ...
        "salary": pd.Series(
            [75000.0, 65000.0, 85000.0], dtype=pd.ArrowDtype(pa.float64())
        ),
    }
